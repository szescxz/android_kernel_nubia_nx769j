...
struct zlog_mod_info goodix_zlog_fp_dev = {
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GF_IOC_RESET:
		...
		gf_hw_reset(gf_dev, RESET_DELAY_TIME);
		break;
...
#ifdef CONFIG_VENDOR_ZTE_DEV_MONITOR_SYSTEM
	gf_dev->zlog_fp_client = zlog_register_client(&goodix_zlog_fp_dev);
	if (gf_dev->zlog_fp_client) {
		pr_info("%s zlog_register_goodixfp_client success\n", __func__);
	} else {
		pr_err("%s zlog_register_goodixfp_client fail\n", __func__);
	}
#endif
	if (status == 0){
		if (status == 0) {
			...
			pr_info("goodix: Succeed to open device. irq = %d\n",
					gf_dev->irq);
			if (gf_dev->users == 1) {
...
#ifdef CONFIG_VENDOR_ZTE_DEV_MONITOR_SYSTEM
	if (gf_dev->zlog_fp_client) {
		zlog_unregister_client(gf_dev->zlog_fp_client);
		pr_info("%s zlog_unregister_client goodix_zlog_fp_dev\n", __func__);
	}
#endif
		...
		unsigned long val, void *data)
{
	...
	if (evdata && evdata->data && gf_dev) {     /*val == FB_EARLY_EVENT_BLANK && */
