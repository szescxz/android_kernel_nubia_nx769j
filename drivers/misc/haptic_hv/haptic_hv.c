...
#endif
#define HAPTIC_HV_DRIVER_VERSION	"v0.5.0"
...
static struct zlog_mod_info zlog_vib_dev = {
        .module_no = ZLOG_MODULE_VIB,
        .name = "vibration",
        .device_name = "vib",
        .ic_name = "aw86927",
        .module_name = "vib_module",
        .fops = NULL,
};
struct zlog_client *zlog_vib_dev_client = NULL;
#endif
/*********************************************************
 *
 * I2C Read/Write
		...
		uint32_t len)
{
	...
	ret = i2c_master_send(aw_haptic->i2c, &reg_addr, AW_I2C_BYTE_ONE);
	if (ret < 0) {
		aw_err("couldn't send addr:0x%02x, ret=%d", reg_addr, ret);
		return ret;
	}
	ret = i2c_master_recv(aw_haptic->i2c, buf, len);
	if (ret != len) {
		aw_err("couldn't read data, ret=%d", ret);
		return ret;
	}
	return ret;
}
	       ...
	       uint8_t reg_data)
{
	...
	if (ret < 0) {
#ifdef CONFIG_VENDOR_ZTE_DEV_MONITOR_SYSTEM
		zlog_client_record(zlog_vib_dev_client, "[vib] aw86927 I2C write failed!");
		zlog_client_notify(zlog_vib_dev_client, ZLOG_VIB_I2C_W_ERR);
#endif
...
static int rtp_osc_cali(struct aw_haptic *aw_haptic)
{
	...
	aw_info("rtp file:[%s] size = %dbytes", aw_rtp_name[0], aw_rtp->len);
	...
	//		   AW_PM_QOS_VALUE_VB);
	cpu_latency_qos_add_request(&aw_pm_qos_req_vb, CPU_LATENCY_QOC_VALUE);
	while (1) {
...
static int ram_select_waveform(struct aw_haptic *aw_haptic)
{
	uint8_t wavseq = 0;
	uint8_t wavloop = 0;
	aw_info("entered ram_select_waveform! duration = %d", aw_haptic->duration);
...
static void richtap_stop_work(struct work_struct *work)
{
	struct aw_haptic *aw_haptic = container_of(work, struct aw_haptic, richtap_stop_work);
...
static long richtap_file_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		case RICHTAP_STOP_MODE:
			queue_work(aw_haptic->work_queue, &aw_haptic->richtap_stop_work);
			break;
	    default:
...
static irqreturn_t irq_handle(int irq, void *data)
{
    ...
    if(atomic_read(&aw_haptic->richtap_rtp_mode) == true)
    {
        ...
        {
            pr_debug("%s: aw_haptic rtp fifo almost empty\n", __func__);
        ...
        {
            aw_dbg("entry MMAP_BUF_DATA_INVALID!");
            aw_haptic->func->set_rtp_aei(aw_haptic, false);
            atomic_set(&aw_haptic->richtap_rtp_mode, false);
        }
...
static enum hrtimer_restart audio_timer_func(struct hrtimer *timer)
{
	...
	ktime_set(aw_haptic->haptic_audio.timer_val / 1000000,
			(aw_haptic->haptic_audio.timer_val % 1000000) * 1000), HRTIMER_MODE_REL);
	return HRTIMER_NORESTART;
}
...
static ssize_t duration_aw_show(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic,
...
static ssize_t duration_aw_store(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t count)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic,
...
static ssize_t activate_aw_show(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic,
...
static ssize_t activate_aw_store(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t count)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic,
...
static ssize_t cont_save_show(struct device *dev,
			    struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	cdev_t *cdev = dev_get_drvdata(dev);
...
static ssize_t cont_save_store(struct device *dev, struct device_attribute *attr,
			     const char *buf, size_t count)
{
	uint32_t val = 0;
	int rc = 0;
...
static void haptic_init(struct aw_haptic *aw_haptic)
{
	...
	aw_haptic->func->enable_gain(aw_haptic);
...
static int aw_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	INIT_WORK(&aw_haptic->richtap_stop_work, richtap_stop_work);
	atomic_set(&aw_haptic->richtap_rtp_mode, false);

    ret = misc_register(&richtap_misc);
    if (ret) {
        aw_err("misc dev failed ret=%d", ret);
    } else {
        aw_err("misc dev is success!");
    }
...
err_irq_config:
	if (gpio_is_valid(aw_haptic->irq_gpio))
		gpio_free(aw_haptic->irq_gpio);

err_irq_gpio_request:
	if (gpio_is_valid(aw_haptic->reset_gpio))
		gpio_free(aw_haptic->reset_gpio);
...
static int __init aw_i2c_init(void)
{
	...
	aw_info("aw_haptic driver version %s", HAPTIC_HV_DRIVER_VERSION);
#ifdef CONFIG_VENDOR_ZTE_DEV_MONITOR_SYSTEM
       if (NULL == zlog_vib_dev_client) {
           zlog_vib_dev_client = zlog_register_client(&zlog_vib_dev);
           if (zlog_vib_dev_client) {
               aw_info("aw_haptic driver register client zlog_vib_dev_client fail!\n");
           }
       }
#endif
	...
	if (ret) {
#ifdef CONFIG_VENDOR_ZTE_DEV_MONITOR_SYSTEM
		zlog_client_record(zlog_vib_dev_client, "[vib] aw86927 I2C read failed!");
		zlog_client_notify(zlog_vib_dev_client, ZLOG_VIB_I2C_R_ERR);
#endif
