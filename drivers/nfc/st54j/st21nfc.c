...
#ifdef CONFIG_VENDOR_ZTE_DEV_MONITOR_SYSTEM
struct zlog_client *zlog_st21nfc_client = NULL;
static struct zlog_mod_info zlog_st21nfc_dev = {
	.module_no = ZLOG_MODULE_NFC,
	.name = "NFC",
	.device_name = "st21nfc_nfc",
	.ic_name = "st54j",
	.module_name = "ST",
	.fops = NULL,
};
	...
	if (st21nfc_dev->clk_run == false) {
		...
		if (ret)
		{
			...
			if(zlog_st21nfc_client){
				zlog_client_record(zlog_st21nfc_client, "st21nfc clock enable error ret=%d", ret);
				zlog_client_notify(zlog_st21nfc_client, ZLOG_NFC_CLK_ENABLE_ERROR_NO);
			}
#endif
			goto err_clk;
		}
	...
	*/
	if(ret < 0 && ret != -107){
		...
		if(zlog_st21nfc_client){
			...
			zlog_client_notify(zlog_st21nfc_client, ZLOG_NFC_I2C_READ_ERROR_NO);
		}
#endif
	}
				 ...
				 size_t count, loff_t *offset)
{
	...
	/*
	Error when ret=-107 can be ignored in ZLOG_NFC_I2C_WRITE_ERROR_NO.
	This is an error that will be reported during the initial connection with i2c.
	*/
	if(ret < 0 && ret != -107){
#ifdef CONFIG_VENDOR_ZTE_DEV_MONITOR_SYSTEM
		if(zlog_st21nfc_client){
			zlog_client_record(zlog_st21nfc_client, "st21nfc i2c master send error ret=%d", ret);
			zlog_client_notify(zlog_st21nfc_client, ZLOG_NFC_I2C_WRITE_ERROR_NO);
		}
#endif
	}
...
// QCOM and MTK54 use standard GPIO definition
...
#ifdef CONFIG_VENDOR_ZTE_DEV_MONITOR_SYSTEM
	zlog_st21nfc_client = zlog_register_client(&zlog_st21nfc_dev);
	if(!zlog_st21nfc_client){
		pr_err("zlog register client zlog_st21nfc_dev fail\n");
	}
#endif
	...
	pr_info("%s complete successfully\n", __func__);
