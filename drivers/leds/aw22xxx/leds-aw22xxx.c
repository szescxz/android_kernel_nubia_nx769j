         ...
         unsigned char reg_addr, unsigned char reg_data)
{
    ...
    while(cnt < AW_I2C_RETRIES) {
        ...
        if(ret < 0) {
            pr_err("%s: i2c_write cnt=%d error=%d, enable=%d\n", __func__, cnt, ret, gpio_get_value(aw22xxx->reset_gpio));
        } else {
            break;
        }
        if (cnt > 0) {
            gpio_direction_output(aw22xxx->reset_gpio, 1);
            pr_info("%s: reset i2c !", __func__);
        }
        cnt ++;
        ...
        struct device_node *np)
{
    ...
    if (aw22xxx->reset_gpio < 0) {
        ...
        if (!ret) {
             dev_info(dev, "%s: not enable gpio, aw22xxx->reset_gpio=%d\n", __func__, aw22xxx->reset_gpio);
        }
...
static int aw22xxx_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
    ...
    ret = gpio_direction_output(aw22xxx->reset_gpio, 0);
...
static int __init aw22xxx_i2c_init(void)
{
    ...
    msleep(500);
    pr_info("wait i2c probe for 0.5s");
...
late_initcall_sync(aw22xxx_i2c_init);
