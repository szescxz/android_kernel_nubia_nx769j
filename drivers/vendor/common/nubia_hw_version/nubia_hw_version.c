...
int nubia_gpio_ctrl(struct platform_device *pdev)
{
	...
	gpio_data = devm_kzalloc(&pdev->dev, sizeof(struct nubia_gpio_pinctrl_info), GFP_KERNEL);
	if (!gpio_data)
		return -ENOMEM;
	...
	gpio_data->pull_up_default = pinctrl_lookup_state(gpio_data->pinctrl, "pull_up_default");
	...
	gpio_data->pull_down_default = pinctrl_lookup_state(gpio_data->pinctrl, "pull_down_default");
	...
	ret = pinctrl_select_state(gpio_data->pinctrl, gpio_data->pull_up_default);
	if (ret){
...
void nubia_get_hw_pcb_version(char* result)
{
	...
	if(NULL != pts_item)
		...
		snprintf(result, PCB_VER_LENGTH, "%s","unknow");
}
		...
		struct kobj_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PCB_VER_LENGTH, "%s", buf);
}
...
static ssize_t nubia_charge_version_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
    char pcb_ext[3][12]={"NX769J_V2A","NX769J_V1A","NX769J_V1B"};

    if(NULL == buf)
		return 0;

    switch(nubia_pcb_gpio3_v)
    {
        case NUBIA_GPIO_PULL_DOWN:
            strcpy(buf,pcb_ext[NUBIA_GPIO_PULL_DOWN]);
            break;
        case NUBIA_GPIO_PULL_UP:
            strcpy(buf,pcb_ext[NUBIA_GPIO_PULL_UP]);
            break;
        case NUBIA_GPIO_FLOAT:
            strcpy(buf,pcb_ext[NUBIA_GPIO_FLOAT]);
            break;
        default:
            break;
    }
	nubia_hw_version_debug("nubia_charge_version=%s\n",buf);
	return snprintf(buf, CONFIG_STRING_LENGTH + 4, "%s", buf);
}
static struct kobj_attribute charge_version_attr=
	__ATTR(charge_version, 0664, nubia_charge_version_show, NULL);

//add by zte to distinguish high medium low settings start
		...
		struct kobj_attribute *attr, char *buf)
{
	...
	switch(nubia_pcb_gpio3_v)
	{
		case NUBIA_GPIO_PULL_DOWN:  //NUBIA_GPIO_PULL_DOWN:0 80W->1
			hml_version = hml_cfg[NUBIA_GPIO_PULL_DOWN];
			break;
		case NUBIA_GPIO_PULL_UP:  //NUBIA_GPIO_PULL_UP:2 65->0
			hml_version = hml_cfg[NUBIA_GPIO_PULL_UP];
			break;
		case NUBIA_GPIO_FLOAT:  //NUBIA_GPIO_FLOAT:1 165->2
			hml_version = hml_cfg[NUBIA_GPIO_FLOAT];
			break;
		default:
			break;
	}
	nubia_hw_version_debug("hml_config_version=%d\n", hml_version);
	return snprintf(buf, sizeof(int) + 1, "%d", hml_version);
}
#ifdef CONFIG_ZTE_FEATURE_CHG_BATT_2S
//add for skin thermal 65W  165W different
int charger_165W (void){
	return (nubia_pcb_gpio3_v == NUBIA_GPIO_FLOAT);
}
int charger_65W (void){
	return !charger_165W();
}
EXPORT_SYMBOL(charger_165W);
EXPORT_SYMBOL(charger_65W);
#endif
...
static ssize_t nubia_hw_pcb_trans_show(struct kobject *kobj,
                struct kobj_attribute *attr, char *buf)
{
    char pcb_trans[3][12]={"NULL","NX679J","NX679J_V1E"};

    if(NULL == buf)
                return 0;

    switch(nubia_pcb_gpio4_v)
    {
        case NUBIA_GPIO_PULL_DOWN:
            strcpy(buf,pcb_trans[NUBIA_GPIO_PULL_DOWN]);
            break;
        case NUBIA_GPIO_PULL_UP:
            strcpy(buf,pcb_trans[NUBIA_GPIO_PULL_UP]);
            break;
        case NUBIA_GPIO_FLOAT:
            strcpy(buf,pcb_trans[NUBIA_GPIO_FLOAT]);
            break;
        default:
            break;
    }
        nubia_hw_version_debug("pcb_ver_trans=%s\n",buf);
        return snprintf(buf, CONFIG_STRING_LENGTH, "%s", buf);
}
static struct kobj_attribute pcb_version_trans_attr=
        __ATTR(pcb_version_trans, 0664, nubia_hw_pcb_trans_show, NULL);
		...
		struct kobj_attribute *attr, char *buf)
{
	...
	return snprintf(buf, RF_BAND_LENGTH, "%s", rf_buf);
}
...
void nubia_get_config_standard(char* result)
{
	...
	if(pts_item != NULL){
		...
		snprintf(result, CONFIG_STRING_LENGTH, "%s", "unknow");
}
		...
		struct kobj_attribute *attr, char *buf)
{
	...
	return snprintf(buf, WIFI_STRING_LENGTH, "%s", wifi_type);
}
...
static ssize_t nfc_config_type_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
	int nfc_gpio  = -1;
	...
	nfc_gpio = nubia_get_gpio_status(nubia_pcb_gpio3_v);
	pr_err("nfc gpio is %d\r\n",nfc_gpio);

	return snprintf(buf, sizeof(int) + 1, "%d", nfc_gpio);
}

static struct kobj_attribute nfc_confi_attr =
	__ATTR(nfc_version, 0664, nfc_config_type_show, NULL);
...
static int  nubia_parse_hw_ver_gpio_dt(struct device_node *node,int* pcb_gpio1_v, int* pcb_gpio2_v)
{
	...
	*pcb_gpio2_v = pcb_gpio2;
	...
	if (!gpio_is_valid(pcb_gpio3)){
		nubia_hw_version_debug("TLMM pcb gpio3 not found\n");
		//return -EPROBE_DEFER;
		return 0;
	}

	rc = gpio_request(pcb_gpio3, "NUBIA_HW_PCB_GPIO3");
	if (rc < 0){
		printk("Failed to request GPIO:%d, ERRNO:%d\n", (s32)pcb_gpio3, rc);
		rc = -ENODEV;
	}
        pcb_gpio4 = of_get_named_gpio(node, "qcom,pcb-gpio4", 0);
        if (!gpio_is_valid(pcb_gpio4)){
                nubia_hw_version_debug("TLMM pcb gpio3 not found\n");
                //return -EPROBE_DEFER;
                return 0;
        }

        rc = gpio_request(pcb_gpio4, "NUBIA_HW_PCB_GPIO4");
        if (rc < 0){
                printk("Failed to request GPIO:%d, ERRNO:%d\n", (s32)pcb_gpio4, rc);
                rc = -ENODEV;
        }
...
//if use pm gpio
	...
	nubia_pcb_gpio3_v += nubia_get_gpio_status(pcb_gpio3);
	nubia_pcb_gpio4_v += nubia_get_gpio_status(pcb_gpio4);
	nubia_hw_version_debug("nubia_pcb_gpio1_v=%x,nubia_pcb_gpio2_v=%x,nubia_pcb_gpio3_v=%x\n",nubia_pcb_gpio1_v,nubia_pcb_gpio2_v,nubia_pcb_gpio3_v);
	{
		const struct hw_pcb_gpio_map_str *pts_item;
		pts_item = nubia_get_pcb_table_item_by_gpio(hw_pcb_gpio_map,
							ARRAY_SIZE(hw_pcb_gpio_map));
		if(NULL != pts_item)
			printk(KERN_ERR"pcb version: %s", pts_item->pcb_ver);
		else
