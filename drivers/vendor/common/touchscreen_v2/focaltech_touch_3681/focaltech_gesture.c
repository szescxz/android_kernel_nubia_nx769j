...
#define GESTURE_DOUBLE                          0x24
#define GESTURE_SINGLE                          0x25
 ...
 *
 */
static DEVICE_ATTR(fts_gesture_mode, 0644, fts_gesture_show,
					 fts_gesture_store);
/*
...
static DEVICE_ATTR(fts_gesture_bm, 0644,
...
static void fts_gesture_report(struct fts_ts_data *ts_data, struct input_dev *input_dev, int gesture_id)
{
	...
	} else if (gesture_id == GESTURE_DOUBLE && ts_data->ztec.is_wakeup_gesture) {
		FTS_INFO("enter:double_gesture");
		ufp_report_gesture_uevent(DOUBLE_TAP_GESTURE);
	} else if (gesture_id == GESTURE_SINGLE) {
		FTS_INFO("enter:single_gesture");
...
void fts_gesture_recovery(struct fts_ts_data *ts_data)
{
    if (ts_data->gesture_support && ts_data->suspended) {
		...
		if (ts_data->ztec.is_single_fp) {
			fts_write_reg(FTS_REG_FOD_EN, FTS_REG_FOD_VALUE);
			FTS_INFO("FOD recovery wirte 0x2 success");
		}
		if (ts_data->ztec.is_wakeup_gesture) {
			fts_write_reg(FTS_REG_DOUBLE_EN, 0x30);
			FTS_INFO("DOUBLE recovery wirte 30 success");
		}
	}
}
...
int fts_gesture_suspend(struct fts_ts_data *ts_data)
{
	...
	for (i = 0; i < 5; i++) {
		fts_write_reg(0xD1, 0x20);/*sinle_tap no delay*/
		fts_write_reg(0xD2, 0xFF);
		fts_write_reg(0xD5, 0xFF);
		fts_write_reg(0xD6, 0xFF);
		fts_write_reg(0xD7, 0xFF);
	...
	{
	  fts_write_reg(FTS_REG_FOD_EN, FTS_REG_FOD_VALUE);
	  FTS_INFO("FOD wirte 0x2 success");
	}

	if (ts_data->ztec.is_wakeup_gesture)
	{
	  fts_write_reg(FTS_REG_DOUBLE_EN, 0x30);
	  FTS_INFO("DOUBLE wirte 0x30 success");
	}
