...
int get_cb_sc(int byte_num, int *cb_buf, enum byte_mode mode)
{
	...
	cb = fts_malloc(byte_num * sizeof(u8));
	if (cb == NULL) {
...
bool get_fw_wp(u8 wp_ch_sel, enum wp_type water_proof_type)
{
	...
	case WATER_PROOF_OFF_TX:
		 ...
		 */
		fw_wp_state = (((wp_ch_sel & 0x03)  == 0x0) || ((wp_ch_sel & 0x03) == 0x02));
		break;
	case WATER_PROOF_OFF_RX:
		fw_wp_state = (((wp_ch_sel & 0x03) == 0x01) || ((wp_ch_sel & 0x03) == 0x02));
		break;
	default:
		break;
	}
...
static void fts_test_save_data_csv(struct fts_test *tdata)
{
	...
	for (i = 0; i < td->item_count; i++) {
		...
		for (j = 1; j <= data_count; j++) {
			...
			line2_length += snprintf(line2_buffer + line2_length,
						 ...
						 "%s, %d, %d, %d, %d, %d, ",
						 info->name, info->code, tx, rx, start_line, index);
			start_line += tx;
			csv_item_count++;
		}
	}
	...
	csv_length += snprintf(csv_buffer + csv_length,
			       CSV_BUFFER_LEN - csv_length, "TestItem Num, %d, ", csv_item_count);

	if (line2_length > 0) {
		csv_length += snprintf(csv_buffer + csv_length,
				       CSV_BUFFER_LEN - csv_length, "%s", line2_buffer);
	}
...
int fts_test_init_basicinfo(struct fts_test *tdata)
{
	int ret = 0;
	u8 val = 0;
	...
	if (tdata == NULL) {
		FTS_TEST_SAVE_ERR("tdata is NULL\n");
	} else if (tdata->func == NULL) {
		FTS_TEST_SAVE_ERR("func is NULL\n");
	}
...
*/
static DEVICE_ATTR(fts_test, 0664, fts_test_show, fts_test_store);
