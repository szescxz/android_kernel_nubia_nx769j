...
static int get_scap_noise(struct fts_test *tdata)
{
	int ret = 0;
	bool fw_wp_check = false;
	int *scap_noise = NULL;
	...
	if (thr->basic.scap_rawdata_wp_on_check && fw_wp_check) {
		...
		noise_tmp = scap_noise + srawdata_cnt;
		ret = get_noise_mc_sc(WATER_PROOF_ON, noise_tmp);
		if (ret < 0) {
	...
	if (thr->basic.scap_rawdata_wp_off_check && fw_wp_check) {
		...
		noise_tmp = scap_noise + srawdata_cnt;
		ret = get_noise_mc_sc(WATER_PROOF_OFF, noise_tmp);
		if (ret < 0) {
	...
	if (thr->basic.scap_rawdata_hi_check && hov_high) {
		...
		noise_tmp = scap_noise + srawdata_cnt;
		ret = get_noise_mc_sc(HIGH_SENSITIVITY, noise_tmp);
		if (ret < 0) {
...
static void save_data_ft5662(char *buf, int *data_length)
{
    ...
    if (true == test_item->noise_test) {
		...
		if (tdata->csv_item_snoise & 0x01) {
            cnt += snprintf(buf + cnt, CSV_BUFFER_LEN - cnt, "%s, %d, %d, %d, %d, %d, ", "SCAP Noise Test", 37, 2, sc_rx, line_num, 1);
            line_num += 2;
        }

        if (tdata->csv_item_snoise & 0x02) {
            cnt += snprintf(buf + cnt, CSV_BUFFER_LEN - cnt, "%s, %d, %d, %d, %d, %d, ", "SCAP Noise Test", 37, 2, sc_rx, line_num, 2);
            line_num += 2;
        }

        if (tdata->csv_item_snoise & 0x04) {
            cnt += snprintf(buf + cnt, CSV_BUFFER_LEN - cnt, "%s, %d, %d, %d, %d, %d, ", "SCAP Noise Test", 37, 2, sc_rx, line_num, 3);
            line_num += 2;
        }
    }
