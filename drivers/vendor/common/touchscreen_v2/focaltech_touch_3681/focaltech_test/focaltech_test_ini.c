	...
	for (i = 0; i < ini->section_num; i++) {
		...
		for (j = 0; j < section->keyword_num; j++) {
			...
			if (key_len == strlen(keyword->name)) {
				if (fts_strncmp(keyword->name, key_name, key_len) == 0) {
...
static int get_test_threshold_incell(void)
{
	...
	/* init buffer */
	fts_init_buffer(thr->rawdata_max, thr->basic.rawdata_max, node_num, raw_key_check, thr->basic.rawdata_max_vk,
			key_num);
	fts_init_buffer(thr->rawdata_min, thr->basic.rawdata_min, node_num, raw_key_check, thr->basic.rawdata_min_vk,
			key_num);
...
static int get_test_threshold_mc_sc(void)
{
    ...
    fts_init_buffer(thr->mcap_cmb_max, thr->basic.mcap_cmb_max, node_num, false, 0, 0);
    fts_init_buffer(thr->mcap_cmb_min, thr->basic.mcap_cmb_min, node_num, false, 0, 0);
    ...
    fts_init_buffer(thr->scap_cb_off_cf_min, thr->basic.scap_cb_off_cf_min, sc_num, false, 0, 0);
    fts_init_buffer(thr->scap_cb_off_cf_max, thr->basic.scap_cb_off_cf_max, sc_num, false, 0, 0);
    fts_init_buffer(thr->scap_cb_on_cf_min, thr->basic.scap_cb_on_cf_min, sc_num, false, 0, 0);
    fts_init_buffer(thr->scap_cb_on_cf_max, thr->basic.scap_cb_on_cf_max, sc_num, false, 0, 0);
    fts_init_buffer(thr->scap_cb_hi_cf_min, thr->basic.scap_cb_hi_cf_min, sc_num, false, 0, 0);
    fts_init_buffer(thr->scap_cb_hi_cf_max, thr->basic.scap_cb_hi_cf_max, sc_num, false, 0, 0);
    ...
    get_detail_threshold("McapCMBTest_Min_Hole", true, thr->mcap_cmb_min, node_num);
    get_detail_threshold("McapCMBTest_Max_Hole", true, thr->mcap_cmb_max, node_num);
...
static void print_thr_mc_sc(void)
{
    ...
    print_buffer(thr->mcap_cmb_min, tdata->node.node_num, tdata->node.rx_num);
    print_buffer(thr->mcap_cmb_max, tdata->node.node_num, tdata->node.rx_num);
    ...
    print_buffer(thr->scap_cb_off_cf_min, tdata->sc_node.node_num, tdata->sc_node.rx_num);
    print_buffer(thr->scap_cb_off_cf_max, tdata->sc_node.node_num, tdata->sc_node.rx_num);
    print_buffer(thr->scap_cb_on_cf_min, tdata->sc_node.node_num, tdata->sc_node.rx_num);
    print_buffer(thr->scap_cb_on_cf_max, tdata->sc_node.node_num, tdata->sc_node.rx_num);
    print_buffer(thr->scap_cb_hi_cf_min, tdata->sc_node.node_num, tdata->sc_node.rx_num);
    print_buffer(thr->scap_cb_hi_cf_max, tdata->sc_node.node_num, tdata->sc_node.rx_num);
...
static u32 ini_get_ic_code(char *ic_name)
{
	...
	for (i = 0; i < type_size; i++) {
		...
		if (ini_icname_len == ic_types_len) {
			if (strncmp(ic_name, ic_types[i].ic_name, ic_types_len) == 0)
