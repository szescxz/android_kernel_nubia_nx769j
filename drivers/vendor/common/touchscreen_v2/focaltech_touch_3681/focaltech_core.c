...
#if defined(CONFIG_TOUCHSCREEN_UFP_MAC)
		uf_touch_report(false, -1, -1, events[finger_count].id);
#endif
#if defined(CONFIG_TOUCHSCREEN_UFP_MAC) && defined(ZTE_ONE_KEY)
		one_key_report(false, -1, -1, events[finger_count].id);
#endif
#ifdef FTS_REPORT_BY_ZTE_ALGO
		if (tpd_cdev->zte_tp_algo)
			tpd_touch_release(ts_data->input_dev, events[finger_count].id);
		else
...
#ifdef FTS_REPORT_BY_ZTE_ALGO
		if (!tpd_cdev->zte_tp_algo){
			input_mt_slot(ts_data->input_dev, events[i].id);
		}
#endif
        if (EVENT_DOWN(events[i].flag)) {
#ifdef FTS_REPORT_BY_ZTE_ALGO
			if (tpd_cdev->zte_tp_algo) {
				tpd_touch_press(ts_data->input_dev, events[i].x, events[i].y, events[i].id,  events[i].area, events[i].p);
			} else {
				input_mt_slot(ts_data->input_dev, events[i].id);
				input_mt_report_slot_state(ts_data->input_dev, MT_TOOL_FINGER, true);
				input_report_abs(ts_data->input_dev, ABS_MT_TOUCH_MAJOR, events[i].area);
				input_report_abs(ts_data->input_dev, ABS_MT_POSITION_X, events[i].x);
				input_report_abs(ts_data->input_dev, ABS_MT_POSITION_Y, events[i].y);
			}
#else	
			input_mt_report_slot_state(ts_data->input_dev, MT_TOOL_FINGER, true);
			input_report_abs(ts_data->input_dev, ABS_MT_TOUCH_MAJOR, events[i].area);
...
#if defined(CONFIG_TOUCHSCREEN_UFP_MAC)
			uf_touch_report(true,events[i].x, events[i].y, events[i].id);
#endif
#if defined(CONFIG_TOUCHSCREEN_UFP_MAC) && defined(ZTE_ONE_KEY)
			one_key_report(true, events[i].x, events[i].y, events[i].id);
#endif
        } else {
#if defined(CONFIG_TOUCHSCREEN_UFP_MAC)
			uf_touch_report(false, -1, -1, events[i].id);
#endif
#if defined(CONFIG_TOUCHSCREEN_UFP_MAC) && defined(ZTE_ONE_KEY)
			one_key_report(false, -1, -1, events[i].id);
#endif
#ifdef FTS_REPORT_BY_ZTE_ALGO
			if (tpd_cdev->zte_tp_algo)
				tpd_touch_release(ts_data->input_dev, events[i].id);
			else
				input_mt_report_slot_state(ts_data->input_dev, MT_TOOL_FINGER, false);
#else
			input_mt_report_slot_state(ts_data->input_dev, MT_TOOL_FINGER, false);
...
#ifdef FTS_REPORT_BY_ZTE_ALGO
				if (tpd_cdev->zte_tp_algo) {
					tpd_touch_release(ts_data->input_dev, i);
				} else {
...
static inline void __report_large_area_uevent(char *str)
{
	...
	if (!ufp_tp_ops.uevent_pdev) {
		UFP_ERR("large uevent pdev is null!\n");
		return;
	}
...
static int fts_irq_read_report(struct fts_ts_data *ts_data)
{
	...
	if (aod_down_flag) {
		...
		if ((ts_data->fod_info.fp_down) && (!ts_data->fod_info.event_type)) {
			ts_data->fod_info.fp_down_report = 1;
			report_ufp_uevent(UFP_FP_DOWN);
			/*input_report_key(input_dev, gesture, 1);
			input_sync(input_dev);*/
		} else if ((!ts_data->fod_info.fp_down) && (ts_data->fod_info.event_type)) {
			ts_data->fod_info.fp_down_report = 0;
			report_ufp_uevent(UFP_FP_UP);
			if (!ts_data->suspended) {
				fts_write_reg(FTS_REG_FOD_EN, FTS_REG_WORKMODE);
				FTS_INFO("CF wirte 0x0 success");
			}
	...
	if (ts_data->ztec.is_palm_mode) {
		fts_write_reg(0x9A, 0x05);
		fts_read_reg(0X9B, &id);
		if (id) {
...
static int focal_ts_power_init(struct fts_ts_data *ts_data)
{
	struct fts_ts_platform_data *ts_bdata = ts_data->pdata;
	struct device *dev = ts_data->dev;
	int ret = 0;

	FTS_FUNC_ENTER();

	ts_data->avdd_enable_gpio = ts_bdata->avdd_enable_gpio;
	ret = devm_gpio_request_one(dev, ts_data->avdd_enable_gpio,
						GPIOF_OUT_INIT_LOW, "TP_AVDD_GPIO");
	if (ret) {
		FTS_ERROR("failed to request avdd_enable_gpio");
		return ret;
	}

	if (strlen(ts_bdata->iovdd_name)) {
		ts_data->iovdd = devm_regulator_get(dev,
				 ts_bdata->iovdd_name);
		if (IS_ERR_OR_NULL(ts_data->iovdd)) {
			ret = PTR_ERR(ts_data->iovdd);
			FTS_ERROR("Failed to get regulator iovdd:%d", ret);
			ts_data->iovdd = NULL;
		}
		ret = regulator_set_voltage(ts_data->iovdd, 1800000, 1800000);
		if (ret < 0) {
			FTS_ERROR("set iovdd voltage failed");
			return ret;
		}
	} else {
...
static int fts_power_source_ctrl(struct fts_ts_data *ts_data, int enable)
{
	...
	if (enable) {
		if (ts_data->power_disabled) {
			...
			ret = gpio_direction_output(ts_data->avdd_enable_gpio, 1);
			if (ret) {
		...
		if (!ts_data->power_disabled) {
			...
			ret = gpio_direction_output(ts_data->avdd_enable_gpio, 0);
			if (ret) {
			...
			if (ret) {
				...
				FTS_INFO("power disable SUCCESS");
			}
...
static int fts_power_source_exit(struct fts_ts_data *ts_data)
{
	...
	if (!ret)
		...
		FTS_ERROR("Failed to disable avdd power:%d", ret);
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
	...
	/*power parse*/
	ret = of_get_named_gpio(np, "avdd-enable-gpio", 0);
	if (ret < 0) {
		FTS_ERROR("invalid avdd-enable-gpio in dt: %d", ret);
		return -EINVAL;
	}
	FTS_INFO("get avdd-enable-gpio[%d] from dt", ret);
	pdata->avdd_enable_gpio = ret;
	
	memset(pdata->iovdd_name, 0, sizeof(pdata->iovdd_name));
	ret = of_property_read_string(np, "focal,iovdd-name", &name_tmp);
	if (!ret) {
...
static void fts_ts_register_for_panel_events(struct fts_ts_data *ts_data)
{
	void *cookie = NULL;
...
static int drm_check_dt(struct device_node *np)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (node != NULL)
			FTS_INFO("%s: node = %s", __func__, node->name);
		...
		if (PTR_ERR(panel) == -ENODEV) {
			FTS_ERROR("%s: no device!", __func__);
			retval = -ENODEV;
		} else if (PTR_ERR(panel) == -EPROBE_DEFER) {
			FTS_ERROR("%s: device has not been probed yet", __func__);
			retval = -EPROBE_DEFER;
		}
		of_node_put(node);
	}
...
#if defined(CONFIG_TOUCHSCREEN_DRM_PANEL_NOTIFIER)
static void fts_ts_panel_notifier_callback(enum panel_event_notifier_tag tag,
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
	...
	fts_ts_register_for_panel_events(ts_data);
	...
	ret = focal_ts_power_init(ts_data);
	if (ret) {
...
int fts_ts_suspend(struct device *dev)
{
	...
	status = (ts_data->ztec.is_wakeup_gesture << 1) | ts_data->ztec.is_single_tap;
...
int fts_ts_resume(struct device *dev)
{
	...
	ts_data->ztec.is_wakeup_gesture = ts_data->ztec.is_set_wakeup_in_suspend;
	ts_data->ztec.is_one_key = ts_data->ztec.is_set_onekey_in_suspend;
#else
	ts_data->ztec.is_single_tap = (ts_data->ztec.is_single_aod || ts_data->ztec.is_single_fp) ? 5 : 0;
	ts_data->ztec.is_wakeup_gesture = ts_data->ztec.is_set_wakeup_in_suspend;
#endif

	FTS_INFO("core_data->ztec.is_single_tap is %d", ts_data->ztec.is_single_tap);
	FTS_INFO("core_data->ztec.is_single_fp is %d", ts_data->ztec.is_single_fp);
	FTS_INFO("core_data->ztec.is_wakeup_gesture is %d", ts_data->ztec.is_wakeup_gesture);
	FTS_INFO("core_data->ztec.is_one_key is %d", ts_data->ztec.is_one_key);

	if (!ts_data->ic_info.is_incell) {
#if FTS_POWER_SOURCE_CUST_EN
		fts_power_source_resume(ts_data);
#endif
		if (aod_down_flag) {  /*(ts_data->ztec.is_one_key)*/
			FTS_INFO("enter resume aod_aremeet_down");
		} else {
			fts_reset_proc(ts_data, 200);
		}
	}
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ts_data->ztec.is_single_tap = 0;
	ts_data->ztec.is_single_aod = 0;
	ts_data->ztec.is_single_fp = 0;
	ts_data->ztec.is_set_single_in_suspend = 0;
	ts_data->ztec.is_set_wakeup_in_suspend = 0;
	ts_data->ztec.is_wakeup_gesture = 0;
	ts_data->ztec.is_one_key = 0;
	ts_data->ztec.is_set_onekey_in_suspend = 0;
	ts_data->ztec.is_play_game = 0;
	ts_data->ztec.is_palm_mode = 0;
#ifdef FOR_ZTE_CELL
	ts_data->ztec.is_smart_cover = 0;
	ts_data->ztec.tp_report_rate = 0;
	FTS_INFO("tp_report_rate set 120HZ");
#else
	ts_data->ztec.tp_report_rate = 1;
	FTS_INFO("tp_report_rate set 240HZ");
#endif
/*
	ts_data->ztec.is_sensibility = 1;
	ts_data->ztec.is_wake_irq = 0;
	ts_data->ztec.finger_lock_flag = 0;
	ts_data->ztec.level = 0;
*/
