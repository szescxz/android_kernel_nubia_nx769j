	...
	char *ptr, s32 *data, s32 rows)
{
	...
	row_data = kzalloc(MAX_LINE_LEN, GFP_KERNEL);
	if (!row_data) {
...
static int goodix_init_testlimits(struct goodix_ts_test *ts_test)
{
	...
	snprintf(limit_file, sizeof(limit_file), "%s_test_limits_%d.csv", GOODIX_TEST_FILE_NAME,
		...
		struct file *fp)
{
	...
	bytes += snprintf(&data[bytes], MAX_DATA_BUFFER, "<OrderConfig>\n");
	for (i = 0; i < cfg->len; i++) {
		bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "0x%02x,", cfg->data[i]);
	}
	bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "\n");
	bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "</OrderConfig>\n");
		...
		struct file *fp)
{
	...
	bytes += snprintf(&data[bytes], MAX_DATA_BUFFER, "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n");
	bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<TESTLOG>\n");
	bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<Header>\n");
	/* sava test result */
	...
	if (result)
		bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<Result>NG</Result>\n");
	else
		bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<Result>OK</Result>\n");
	bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<DeviceType>GT%s</DeviceType>\n",
			ts->fw_version.patch_pid);
	bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<SensorId>%d</SensorId>\n",
	...
	bytes += snprintf(&data[bytes], MAX_DATA_BUFFER, "</Header>\n");
	ret = fs_write(data, bytes, fp);
	if (ret < 0) {
		ts_err("header write failed");
		goto save_end;
	...
	/* item list */
	bytes += snprintf(&data[bytes], MAX_DATA_BUFFER, "<ItemList>\n");
	...
	bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "</ItemList>\n");
	ret = fs_write(data, bytes, fp);
	if (ret < 0) {
		...
		struct file *fp)
{
	...
	bytes += snprintf(&data[bytes], MAX_DATA_BUFFER, "<TestItems>\n");

	/* save short result */
	if (ts_test->test_result[GTP_SHORT_TEST]) {
		bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<Item name=\"Short Test\">\n");
		bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<ShortNum>%d</ShortNum>\n",
		...
		for (i = 0; i < ts_test->short_res.short_num; i++) {
			...
			if (chn1 == CHN_VDD)
				bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<ShortMess Chn1=\"VDD\" ");
			else if (chn1 == CHN_GND)
				bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<ShortMess Chn1=\"GND\" ");
			else if (chn1 & DRV_CHANNEL_FLAG)
				bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<ShortMess Chn1=\"Tx%d\" ",
						chn1 & 0x7f);
			else
				bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<ShortMess Chn1=\"Rx%d\" ",
						chn1 & 0x7f);
			if (chn2 == CHN_VDD)
				bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes,
		...
		bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "</Item>\n");
		ret = fs_write(data, bytes, fp);
		if (ret < 0) {
	...
	/* rawdata max limit */
	bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<Item name=\"Rawdata Test Sets\">\n");
	bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<TotalFrameCnt>%d</TotalFrameCnt>\n",
			TOTAL_FRAME_NUM);
	bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<MaxRawLimit>\n");
	for (i = 0; i < tx * rx; i++) {
		bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "%d,",
	...
	bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<BeyondRawdataUpperLimitCnt>\n");
	for (i = 0; i < tx * rx; i++) {
		bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "%d,",
	...
	/* rawdata min limit */
	bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<MinRawLimit>\n");
	for (i = 0; i < tx * rx; i++) {
		bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "%d,",
	...
	bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<BeyondRawdataLowerLimitCnt>\n");
	for (i = 0; i < tx * rx; i++) {
		bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "%d,",
	...
	/* Max Accord limit */
	bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<MaxAccordLimit>\n");
	for (i = 0; i < tx * rx; i++) {
		bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "%d,",
	...
	bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<BeyondAccordLimitCnt>\n");
	for (i = 0; i < tx * rx; i++) {
		bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "%d,",
	...
	/* save noise limit */
	if (ts_test->test_result[GTP_NOISE_TEST]) {
		bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<Item name=\"Diffdata Test Sets\">\n");
		bytes += snprintf(&data[bytes],  MAX_DATA_BUFFER-bytes,
			"<TotalFrameCnt>%d</TotalFrameCnt>\n", NOISEDATA_TEST_TIMES);
		bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<MaxJitterLimit>%d</MaxJitterLimit>\n",
	...
	if (ts_test->test_result[GTP_SELFCAP_TEST]) {
		bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<Item name=\"Self Rawdata Test Sets\">\n");
		bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<TotalFrameCnt>1</TotalFrameCnt>\n");
		bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<MaxRawLimit>\n");
		for (i = 0; i < tx + rx; i++) {
			bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "%d,",
					ts_test->test_params.self_max_limits[i]);
	...
	if (ts_test->test_result[GTP_SELFNOISE_TEST]) {
		bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<Item name=\"Self Diffdata Test Sets\">\n");
		bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<TotalFrameCnt>1</TotalFrameCnt>\n");
		bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<MaxJitterLimit>%d</MaxJitterLimit>\n",
	...
	bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "</TestItems>\n");
	ret = fs_write(data, bytes, fp);
	if (ret < 0)
		...
		struct file *fp)
{
	...
	bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<RawDataRecord>\n");
	for (i = 0; i < TOTAL_FRAME_NUM; i++) {
		...
		for (j = 0; j < len; j++) {
			bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "%d,", ts_test->rawdata[i].data[j]);
			if ((j + 1) % tx == 0)
				bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "\n");
		}
		bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "</DataContent>\n");
		goodix_data_cal(ts_test->accord_arr[i].data, len, stat_result);
		bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes,
			"<RawAccord No.=\"%d\" DataCount=\"%d\" Maximum=\"%d\" Minimum=\"%d\" Average=\"%d\">\n",
			i, len, stat_result[1], stat_result[2], stat_result[0]);
		for (j = 0; j < len; j++) {
			bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "%d,", ts_test->accord_arr[i].data[j]);
			if ((j + 1) % tx == 0)
				bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "\n");
		}
		bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "</RawAccord>\n");
		ret = fs_write(data, bytes, fp);
		if (ret < 0) {
...
static int goodix_save_noise_data(struct goodix_ts_test *ts_test, struct file *fp)
{
	...
	bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<DiffDataRecord>\n");
	for (i = 0; i < NOISEDATA_TEST_TIMES; i++) {
		...
		bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes,
				 ...
				 i, len, stat_result[1], stat_result[2], stat_result[0]);
		for (j = 0; j < len; j++) {
			bytes += snprintf(&data[bytes],  MAX_DATA_BUFFER-bytes, "%d,", ts_test->noisedata[i].data[j]);
			if ((j + 1) % tx == 0)
			bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "\n");
		}
		bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "</DataContent>\n");
		...
		struct file *fp, s16 *src_data, u8 *title, int len)
{
	...
	bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<%s>\n", title);
	ret = fs_write(data, bytes, fp);
	if (ret < 0) {
		ts_err("rawdata write fail.");
		goto save_end;
	...
	for (i = 0; i < len; i++) {
		bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "%d,", src_data[i]);
		if ((i + 1) % tx == 0)
			bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "\n");
	}
	if (len % tx != 0)
		...
		struct file *fp)
{
	...
	bytes += snprintf(&data[bytes], MAX_DATA_BUFFER-bytes, "<DataRecord>\n");
	ret = fs_write(data, bytes, fp);
	if (ret < 0) {
...
static void goodix_save_result_data(struct goodix_ts_test *ts_test)
{
	...
	/* format result file */
	snprintf(save_path, sizeof(save_path), GOODIX_RESULT_SAVE_PATH);
