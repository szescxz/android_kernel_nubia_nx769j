...
#ifdef COMPATIBLE_MULTIPLE_CHIPS
	u8 buf[sizeof(struct goodix_fw_version)];

	cd->bus->ic_type = IC_TYPE_BERLIN_D;
	ret = hw_ops->read(cd, FW_VERSION_INFO_ADDR_BRA, buf, sizeof(buf));
	if (ret < 0)
		goto CONFIRM_VAL;
...
static int brl_power_on(struct goodix_ts_core *cd, bool on)
{
	...
	if (on) {
		...
		ret = gpio_direction_output(cd->avdd_enable_gpio, 1);
		if (ret) {
...
power_off:
	...
	ret = gpio_direction_output(cd->avdd_enable_gpio, 0);
	if (!ret)
...
int brl_suspend(struct goodix_ts_core *cd)
{
	...
	ts_info("send sleep cmd");
	if (cd ->hw_ops->write(cd, cmd_reg, sleep_cmd, sizeof(sleep_cmd))) {
...
static int brl_package_config(u8 *cfg, int len, u8 *buf)
{
	...
	buf[10] = (len >> 16) & 0xFF;
	buf[11] = (len >> 24) & 0xFF;
	memcpy(&buf[12], cfg, len);

	for (i = 0; i < len + 8; i += 2)
		checksum += (buf[4 + i] + (buf[5 + i] << 8));
...
static int brl_send_config(struct goodix_ts_core *cd, u8 *cfg, int len)
{
	...
	if ((len > misc->fw_buffer_max_len) && (misc->fw_buffer_max_len > 0)) {
	...
	if (cd->bus->ic_type == IC_TYPE_BERLIN_D) {
		cmd_start = CONFIG_CMD_START_BD;
		cmd_write = CONFIG_CMD_WRITE_BD;
		cmd_exit = CONFIG_CMD_EXIT_BD;
		start_status = CONFIG_CMD_START_STATUS;
		write_status = CONFIG_CMD_WRITE_STATUS;
		tx_buf = buffer;
		tx_len = len + CONFIG_DATA_HEAD_BD;
		ret = brl_package_config(cfg, len, buffer);
		if (ret < 0)
			return ret;
	}
...
/*int zte_brl_gesture(struct goodix_ts_core *cd, int gesture_type_first, int gesture_type_second)
{
	struct goodix_ts_cmd cmd;
	...
	if (cd->bus->ic_type == IC_TYPE_BERLIN_D) {
		...
		cmd.len = 6;
		cmd.data[0] = gesture_type_first;
		cmd.data[1] = gesture_type_second;
	} else {
...
int zte_set_display_rotation(struct goodix_ts_core *cd, int mrotation)
{
	...
	if (!cd->ztec.is_play_game) {
		if (cd->hw_ops->set_tp_report_rate(cd, cd->ztec.tp_report_rate)) {
			...
			ts_err("set report rate mode success!");
		}
	}
	return 0;
}
...
int zte_tp_set_report_rate(struct goodix_ts_core *cd, int enable)
{
	...
	if (cd->bus->ic_type == IC_TYPE_BERLIN_D) {
		...
		switch (enable) {
			case tp_freq_120Hz:
				if (current_report_mode == tp_freq_360Hz) {
					ts_info("%s: current_report_mode is 360HZ, exit 360HZ mode", __func__);
					cmd.cmd = 0xC0;
					cmd.data[0] = 0x00;
					cmd.data[1] = 0xC5;
					cmd.data[2] = 0x0;
				...
				ts_info("%s 120Hz", __func__);
				current_report_mode = tp_freq_120Hz;
				cmd.cmd = 0x9d;
				cmd.data[0] = 0x0;/*120Hz*/
				cmd.data[1] = 0xA2;
				cmd.data[2] = 0x0;
				break;
			case tp_freq_240Hz:
			if (current_report_mode == tp_freq_360Hz) {
				...
				ts_info("%s 240Hz", __func__);
				current_report_mode = tp_freq_240Hz;
				cmd.cmd = 0x9d;
				cmd.data[0] = 0x02;/*240Hz*/
				cmd.data[1] = 0xA4;
				cmd.data[2] = 0x0;
				break;
			case tp_freq_360Hz:
				ts_info("%s 360Hz", __func__);
				current_report_mode = tp_freq_360Hz;
				cmd.cmd = 0xC0;
				cmd.data[0] = 0x01;/*360Hz*/
				cmd.data[1] = 0xC6;
				cmd.data[2] = 0x0;
				break;
			default:
...
int zte_set_tp_pen_only_mode(struct goodix_ts_core *cd, int mode)
{
	struct goodix_ts_cmd cmd;
	...
	cmd.len = 0x5;
	if (mode > 0) {
		ts_info("%s enter pen only mode", __func__);
		cmd.data[0] = 0x01;
		cmd.data[1] = 0xc3;
		cmd.data[2] = 0x0;
	} else {
		ts_info("%s exit pen only mode", __func__);
		cmd.data[0] = 0x0;
		cmd.data[1] = 0xc2;
		cmd.data[2] = 0x0;
	}
...
static bool zte_pen_low_batt_check(struct goodix_ts_core *cd)
{
	int ret;
	u32 pen_low_batt_addr;
	u8 pen_low_batt_value;

	pen_low_batt_addr = 0x1027d;
	ret = cd->hw_ops->read(cd, pen_low_batt_addr, &pen_low_batt_value, 1);
	if (ret) {
		ts_err("failed get esd pen_low_cap_value, %d", ret);
		return false;
	}

	if (pen_low_batt_value & 0x02) {
		ts_info("pen battery is low!");
		return true;
	}
	return false;
}
...
static struct goodix_ts_hw_ops brl_hw_ops = {
	...
	.set_tp_pen_only_mode = zte_set_tp_pen_only_mode,
#ifdef FOR_ZTE_CELL
	.set_enter_smart_cover = zte_enter_smart_cover,
	.set_leave_smart_cover = zte_leave_smart_cover,
#endif
#ifdef PEN_LOW_BATT_CHECK
	.pen_low_batt_check = zte_pen_low_batt_check,
#endif
};
