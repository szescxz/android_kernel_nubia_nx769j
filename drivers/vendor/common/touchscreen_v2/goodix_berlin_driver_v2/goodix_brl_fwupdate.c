...
static int gtp_cfg_bin_upgrade(struct goodix_ts_core *core_data, const struct ts_firmware *gtp_firmware)
{
	struct goodix_cfg_bin cfg_bin = { 0 };
	struct goodix_ic_config *config;
	int ret = 0;

	cfg_bin.bin_data_len = gtp_firmware->size;
	cfg_bin.bin_data = kzalloc(cfg_bin.bin_data_len, GFP_KERNEL);
	if (!cfg_bin.bin_data) {
		ts_err("alloc cfg_bin data failed");
		return -ENOMEM;
	}
	memcpy(cfg_bin.bin_data, gtp_firmware->data, cfg_bin.bin_data_len);
	...
	ret = goodix_get_reg_and_cfg(core_data, core_data->fw_version.sensor_id, &cfg_bin);
	if (!ret) {
		ts_info("success get reg and cfg info from cfg bin");
	} else {
		ts_err("failed get cfg and reg info");
		goto exit;
	}
	...
	if (!config || config->len == 0) {
		...
		ts_info("config %d is exist", CONFIG_TYPE_NORMAL);
	}
...
const struct ts_firmware *gtp_tp_requeset_firmware(char *file_name)
{
	...
	if (firmware == NULL) {
		ts_err("Request from file alloc struct firmware failed");
		return NULL;
	}
	if (!cdev->tp_firmware->size)
		goto err_free_firmware;
	firmware->size = cdev->tp_firmware->size;
	firmware->data = (u8 *)cdev->tp_firmware->data;
	return firmware;
err_free_firmware:
	kfree(firmware);
	return NULL;
...
static int gtp_fw_upgrade(const struct ts_firmware *gtp_firmware)
{
	...
	if (temp_firmware == NULL) {
		ts_err("Request temp_firmware from file alloc struct firmware failed");
		return -EINVAL;
	}
	temp_firmware->data = gtp_firmware->data;
	temp_firmware->size = gtp_firmware->size;
...
int gtp_tp_fw_upgrade(struct ztp_device *cdev, char *fw_name, int fwname_len)
{
	const struct ts_firmware *gtp_firmware = NULL;
	struct goodix_ts_core *core_data = (struct goodix_ts_core *)cdev->private;
	int ret = 0;

	gtp_firmware = gtp_tp_requeset_firmware(NULL);
	if (gtp_firmware == NULL) {
		ts_err("get firmware failed");
		return -EINVAL;
	}
	if (gtp_firmware->size > GOODIX_CFG_MAX_SIZE) {
		ts_info("gtp_fw_upgrade");
		ret = gtp_fw_upgrade(gtp_firmware);
		if (ret) {
			ts_err("gtp fw update failed");
		}
	} else {
		ts_info("gtp_cfg_bin_upgrade");
		ret = gtp_cfg_bin_upgrade(core_data, gtp_firmware);
		if (ret) {
			ts_err("gtp cfg fw update failed");
		}
	}
	kfree(gtp_firmware);
	return ret;
		...
		char *buf)
{
	...
	case UPSTA_PREPARING:
		snprintf(str, GOODIX_MAX_STR_LABLE_LEN, "preparing");
		break;
	case UPSTA_UPDATING:
		snprintf(str, GOODIX_MAX_STR_LABLE_LEN, "updating");
		break;
	case UPSTA_SUCCESS:
		snprintf(str, GOODIX_MAX_STR_LABLE_LEN, "success");
		break;
	case UPSTA_FAILED:
		snprintf(str, GOODIX_MAX_STR_LABLE_LEN, "failed");
		break;
	case UPSTA_NOTWORK:
	default:
		snprintf(str, GOODIX_MAX_STR_LABLE_LEN, "notwork");
		break;
	}
