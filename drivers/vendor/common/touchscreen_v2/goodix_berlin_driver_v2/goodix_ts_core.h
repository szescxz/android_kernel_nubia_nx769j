...
#include <linux/fb.h>
#endif
#include "firmware_config/freyr/goodix_config.h"
...
#define GOODIX_CFG_MAX_SIZE				(4096 * 2)
...
extern char gtp_cfg_firmware_name[];
extern char gtp_firmware_name[];
#define TS_DEFAULT_FIRMWARE				gtp_firmware_name
#define TS_DEFAULT_CFG_BIN				gtp_cfg_firmware_name
enum CORD_PROB_STA {
...
enum GOODIX_IC_CONFIG_TYPE {
	...
	CONFIG_TYPE_ULTRA_HIGHSENSE = 3,
	CONFIG_TYPE_CHARGER = 4,
	CONFIG_TYPE_CHARGER_HS = 5,
	CONFIG_TYPE_HOLSTER = 6,
	CONFIG_TYPE_HOSTER_CH = 7,
	CONFIG_TYPE_OTHER = 8,
	/* keep this at the last */
	GOODIX_MAX_CONFIG_GROUP = 9,
};
...
struct goodix_ts_hw_ops {
	...
	int (*gesture)(struct goodix_ts_core *cd,  int status);
	...
	int (*set_display_rotation)(struct goodix_ts_core *cd, int mrotation);
	int (*set_enter_charger)(struct goodix_ts_core *cd);
	int (*set_leave_charger)(struct goodix_ts_core *cd);
	int (*set_onekey_switch)(struct goodix_ts_core *cd, int onekey_type);
	int (*set_tp_report_rate)(struct goodix_ts_core *cd, int mode);
	int (*set_tp_pen_only_mode)(struct goodix_ts_core *cd, int mode);
	...
	bool (*pen_low_batt_check)(struct goodix_ts_core *cd);
#endif
#ifdef FOR_ZTE_CELL
	int (*set_enter_smart_cover)(struct goodix_ts_core *cd);
	int (*set_leave_smart_cover)(struct goodix_ts_core *cd);
#endif
};
...
struct goodix_ts_core {
...
#endif
	struct workqueue_struct *gtp_ts_workqueue;
	struct work_struct	gtp_resume_work;
#ifdef PEN_LOW_BATT_CHECK
	struct delayed_work pen_low_batt_check_work;
#endif
	...
	struct pinctrl_state *pinctrl_state_init;
#endif
	struct zte_ctl ztec;
