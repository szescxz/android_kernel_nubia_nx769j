...
extern int gtp_get_vendor_and_firmware(void);
#ifdef GTP_PINCTRL_EN
extern int goodix_platform_pinctrl_init(struct goodix_ts_core *core_data);
#endif
...
#if !defined(CONFIG_SOC_SPRD) || defined(CONFIG_SPRD_DEBUG)
/* prosfs create */
...
static inline void point_up_log(int index)
{
	if (finger_down[index]) {
		finger_down[index] = 0;
		pr_info("GTP_touch_point: touch_up id: %d, coord[%d, %d]\n",
...
static inline void point_down_log(int x, int y, int index)
{
	if (!finger_down[index]) {
		finger_down[index] = 1;
		pr_info("GTP_touch_point: touch_down id: %d, coord[%d, %d]\n", index, x, y);
	}
...
{
	int i;
	static u32 pen_average_pressure = 0;
	static u16 count = 0;
	static u16 pressure_judge_up = 0;

	mutex_lock(&dev->mutex);

	if (pen_data->coords.p == 0) {
		pen_average_pressure = 0;
		pressure_judge_up = 0;
		count = 0;
	} else {
		count = count < 10 ?  count : 10;
		if (pen_average_pressure == 0)
			pen_average_pressure = pen_data->coords.p;
		else {
			ts_debug("pen_average_pressure: %d, count: %d, p: %d",
			 	pen_average_pressure, count, pen_data->coords.p);
			pen_average_pressure = (pen_average_pressure * count + pen_data->coords.p) / (count + 1);
		}
		count++;
	}
	if (!pressure_judge_up && (pen_data->coords.p < pen_average_pressure / 3)) {
		pen_data->coords.p = 0;
		pressure_judge_up = pen_average_pressure;
	}
	if (pressure_judge_up && (pen_data->coords.p < pressure_judge_up * 6 / 5))
		pen_data->coords.p = 0;
	else
		pressure_judge_up = 0;
	if (pen_data->coords.status == TS_TOUCH) {
		if (tpd_cdev->display_rotation) {
			if (pen_data->coords.x == 0) {
				pen_data->coords.x = 1;
			}
			if (pen_data->coords.y == 0) {
				pen_data->coords.y = 1;
			}
		}
		...
		point_down_log(pen_data->coords.x, pen_data->coords.y, 0);
		/*ts_info("pen_data:x %d, y %d, p %d, tilt_x %d tilt_y %d key[%d %d]",
		...
		struct goodix_touch_data *touch_data)
{
	...
	if (cdev->zte_tp_algo) {
			tpd_touch_press(dev, touch_data->coords[i].x, touch_data->coords[i].y,
				i, touch_data->coords[i].w, 0);
	} else {
		input_mt_slot(dev, i);
...
#endif
#if defined(CONFIG_TOUCHSCREEN_UFP_MAC) && defined(CONFIG_POINT_SIMULAT_UF)
			uf_touch_report(true,
					touch_data->coords[i].x,
					touch_data->coords[i].y,
					i);
#endif
...
static irqreturn_t goodix_ts_threadirq_func(int irq, void *data)
{
	...
	if (likely(!ret)) {
		...
		if (core_data->board_data.pen_enable &&
			...
			if (hw_ops->pen_low_batt_check(core_data) && core_data->pen_low_batt_enable) {
				core_data->pen_low_batt_enable =  false;
				queue_delayed_work(core_data->gtp_ts_workqueue, &core_data->pen_low_batt_check_work, 300 * HZ);
				if (tpd_cdev->tpd_report_uevent != NULL) {
					ts_info("pen low batt Uevent\n");
					tpd_cdev->tpd_report_uevent(pen_low_batt);
				}
			}
#endif
...
void goodix_ts_touch_power_exit(struct goodix_ts_core *core_data)
{
	...
	if (!IS_ERR_OR_NULL(core_data->iovdd)) {
		regulator_put(core_data->iovdd);
	}
	if (core_data->board_data.avdd_enable_gpio > 0) {
		ts_info("avdd_enable gpio free");
		gpio_free(core_data->board_data.avdd_enable_gpio);
	}
	if (core_data->board_data.iovdd_gpio > 0) {
		ts_info("iovdd gpio free");
		gpio_free(core_data->board_data.iovdd_gpio);
	}
	if (core_data->board_data.avdd_gpio > 0) {
		ts_info("avvdd gpio free");
		gpio_free(core_data->board_data.avdd_gpio);
	}
}
...
static void goodix_ts_release_connects(struct goodix_ts_core *core_data)
{
	...
	int i;
	u32 touch_data_addr = core_data->ic_info.misc.touch_data_addr;
	u8 temp_buf[48] = {0};
	...
	if (gesture_flag) {
		ts_info("clear buff");
		core_data->hw_ops->write(core_data, touch_data_addr, temp_buf, sizeof(temp_buf));
	} else {
		ts_info("close gesture,no need clear buff");
	}
}
...
int goodix_ts_suspend(struct goodix_ts_core *core_data)
{
	...
	ret = cancel_delayed_work_sync(&core_data->pen_low_batt_check_work);
	ts_info("cancel pen low batta check work, state %d", ret);
#endif
	/* inform external module */
	mutex_lock(&goodix_modules.mutex);
	if (!list_empty(&goodix_modules.head)) {
...
out:
	...
	ts_info("core_data->ztec.is_wakeup_gesture << 1 is %d", core_data->ztec.is_wakeup_gesture << 1);
	ts_info("status is %d", status);
#else
	status = core_data->ztec.is_wakeup_gesture << 1;
	ts_info("core_data->ztec.is_wakeup_gesture << 1 is %d", core_data->ztec.is_wakeup_gesture << 1);
...
#endif
 	 	/*if ((core_data->ztec.tp_report_rate != 1) && (core_data->ztec.is_play_game == 0)) {
  		ts_info(" report rate mode is %d", core_data->ztec.tp_report_rate );
	...
	if (core_data->ztec.is_play_game) {
		ts_info("resume open game mode");
		ret = hw_ops->set_zte_play_game(core_data, core_data->ztec.is_play_game);
		if (!ret)
  			ts_info("set game rate mode success");
  		else
  			ts_err("set game rate mode failed!");
	}

	if(tpd_cdev->pen_only_mode) {
		ts_info(" pen only mode is %d", tpd_cdev->pen_only_mode);
		ret = hw_ops->set_tp_pen_only_mode(core_data, tpd_cdev->pen_only_mode);
		if (!ret)
...
#ifdef FOR_ZTE_CELL
	if (smart_cover_flag) {
		reteval = hw_ops->set_enter_smart_cover(core_data);
		ts_info("resume enter_smart_cover success\n");
	} else {
		reteval = hw_ops->set_leave_smart_cover(core_data);
		ts_info("resume set_leave_smart_cover success\n");
	}
#endif
#ifdef CONFIG_TOUCHSCREEN_UFP_MAC
...
static void goodix_ts_register_for_panel_events(struct goodix_ts_core *core_data)
{
	void *cookie = NULL;
...
#ifdef CONFIG_PM
#if !defined(CONFIG_FB) && !defined(CONFIG_HAS_EARLYSUSPEND) && !defined(CONFIG_TOUCHSCREEN_DRM_PANEL_NOTIFIER)
/**
...
static void get_firmware_and_config(struct goodix_ts_core *cd)
{
#if 0
	tpd_ufp_register(cd);
#endif

	if (!strcmp(DEVICE_NODE_NAME, "qcom,mdss_dsi_visionox_rm692e1_full_screen_4th_cmd")) {
		ts_info("firmware_name:gt9897");
		strncpy(cd->board_data.fw_name, "goodix_firmware_gt9897.bin", sizeof(cd->board_data.fw_name));
		strncpy(cd->board_data.cfg_bin_name, "goodix_cfg_group_gt9897.bin", sizeof(cd->board_data.cfg_bin_name));
	} else {
		ts_info("firmware_name:gt9916");
		strncpy(cd->board_data.fw_name, "goodix_firmware_gt9916.bin", sizeof(cd->board_data.fw_name));
		strncpy(cd->board_data.cfg_bin_name, "goodix_cfg_group_gt9916.bin", sizeof(cd->board_data.cfg_bin_name));
	}
}
...
void goodix_resume_work(struct work_struct *work)
{
	struct goodix_ts_core *core_data = container_of(work, struct goodix_ts_core, gtp_resume_work);

	ts_info("goodix_resume_work");
	goodix_ts_resume(core_data);
}

static int tpd_goodix_ts_resume(void *_core_data)
{
	struct goodix_ts_core *core_data = (struct goodix_ts_core *)_core_data;
...
static void tpd_resume_suspend_register(struct goodix_ts_core *core_data)
{
...
static int goodix_ts_probe(struct platform_device *pdev)
{
	...
	if (tpd_cdev->TP_have_registered) {
		ts_info("TP have registered by other TP.\n");
		return -EPERM;
	}
	ts_info("IN");
	...
	ret = goodix_ts_check_dt(bus_interface->dev);
...
#ifdef GTP_PINCTRL_EN
	goodix_platform_pinctrl_init(core_data);
#endif
	...
	if (ret) {
		...
		ts_info("reset success");
	}
	/* confirm it's goodix touch dev or not */

	get_firmware_and_config(core_data);
	...
	core_data->ztec.is_set_single_in_suspend = 0;
	core_data->ztec.is_sensibility = 1;
	core_data->ztec.is_set_wakeup_in_suspend = 0;
	core_data->ztec.is_wakeup_gesture = 0;
	core_data->ztec.is_wake_irq = 0;
	core_data->ztec.is_one_key = 0;
	core_data->ztec.finger_lock_flag = 0;
	core_data->ztec.is_set_onekey_in_suspend = 0;
	core_data->ztec.is_play_game = 0;
#ifdef FOR_ZTE_CELL
	core_data->ztec.is_smart_cover = 0;
...
#endif
	core_data->ztec.level = 0;
	mutex_init(&core_data->mutex_cmd);
...
err_out:
	...
	core_module_prob_sate = CORE_MODULE_PROB_FAILED;
	gpio_free(core_data->board_data.reset_gpio);
	gpio_free(core_data->board_data.irq_gpio);
...
#ifdef CONFIG_TOUCHSCREEN_DRM_PANEL_NOTIFIER
	if (active_panel && core_data->notifier_cookie)
