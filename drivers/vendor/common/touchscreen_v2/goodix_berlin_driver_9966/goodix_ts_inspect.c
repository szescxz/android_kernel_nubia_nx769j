...
static int goodix_init_testlimits(struct goodix_ts_test *ts_test)
{
	...
	snprintf(limit_file, sizeof(limit_file), "%s_%d.csv", GOODIX_TEST_FILE_NAME,
...
static int goodix_save_test_config(struct goodix_ts_test *ts_test)
{
	...
	ret = tpd_copy_to_tp_firmware_data(data);
	if (ret < 0) {
...
int goodix_do_inspect(struct goodix_ts_core *cd,
		struct ts_rawdata_info *info)
{
	int ret;
