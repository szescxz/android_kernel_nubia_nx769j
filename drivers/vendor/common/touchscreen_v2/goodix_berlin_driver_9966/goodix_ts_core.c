		...
		struct goodix_touch_data *touch_data)
{
	...
	for (i = 0; i < GOODIX_MAX_TOUCH; i++) {
		if(cd->ztec.is_fold_state) {
			if ((touch_data->coords[i].status == TS_TOUCH) && (touch_data->coords[i].x > FOLD_CUT_PIX_LEN)) {
				...
				if (cdev->zte_tp_algo) {
					...
					tpd_touch_press(cd->input_dev, touch_data->coords[i].x, touch_data->coords[i].y,
						i, touch_data->coords[i].w, 0);
				} else {
					input_mt_slot(cd->input_dev, i);
					input_mt_report_slot_state(cd->input_dev, MT_TOOL_FINGER, true);
					input_report_abs(cd->input_dev, ABS_MT_POSITION_X, touch_data->coords[i].x);
					input_report_abs(cd->input_dev, ABS_MT_POSITION_Y, touch_data->coords[i].y);
					input_report_abs(cd->input_dev, ABS_MT_TOUCH_MAJOR, touch_data->coords[i].w);
				}
#else
				input_mt_slot(cd->input_dev, i);
				input_mt_report_slot_state(cd->input_dev, MT_TOOL_FINGER, true);
...
#ifdef GTP_REPORT_BY_ZTE_ALGO
				if (cdev->zte_tp_algo) {
					tpd_touch_release(cd->input_dev, i);
				} else {
					input_mt_slot(cd->input_dev, i);
					input_mt_report_slot_state(cd->input_dev, MT_TOOL_FINGER, false);
				}
#else
				input_mt_slot(cd->input_dev, i);
				input_mt_report_slot_state(cd->input_dev, MT_TOOL_FINGER, false);
	...
	if (touch_data->have_key) {
		for (i = 0; i < GOODIX_MAX_KEY; i++) {
			if (touch_data->keys[i].status == TS_TOUCH)
				input_report_key(cd->input_dev, touch_data->keys[i].code, 1);
			else
				input_report_key(cd->input_dev, touch_data->keys[i].code, 0);
		}
	}

	input_report_key(cd->input_dev, BTN_TOUCH, touch_num > 0 ? 1 : 0);
...
#endif
	ret = hw_ops->set_zte_fold_mode(core_data, core_data->ztec.is_fold_state);
		 ...
		 struct panel_event_notification *notification, void *client_data)
{
	...
	case DRM_PANEL_EVENT_UNBLANK:
		...
		if (notification->notif_data.early_trigger)
			...
			UFP_INFO("unblank will goto resume\n");
			change_tp_state(ON);
		}
		break;
	case DRM_PANEL_EVENT_BLANK:
		...
		if (notification->notif_data.early_trigger) {
			UFP_INFO("blank will goto suspend\n");
			change_tp_state(OFF);
		} else {
