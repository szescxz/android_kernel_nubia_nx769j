		...
		char *file_name)
{
	...
	bytes = 0;
	/* save test config */
	ret = goodix_save_test_config(ts_test, file_name);
	if (ret < 0)  {
...
static int goodix_save_self_data(struct goodix_ts_test *ts_test,
		char *file_name, s16 *src_data, u8 *title, int len)
{
	int i;
	int ret = 0;
	s32 bytes = 0;
		...
		char *file_name)
{
	...
	ret = goodix_save_rawdata(ts_test, file_name);
	if (ret < 0)
		goto save_end;

	if (ts_test->test_result[GTP_NOISE_TEST]) {
		ret = goodix_save_noise_data(ts_test, file_name);
		if (ret < 0)
			goto save_end;
	}

	if (ts_test->test_result[GTP_SELFCAP_TEST]) {
		ret = goodix_save_self_data(ts_test, file_name,
...
static void goodix_save_result_data(struct goodix_ts_test *ts_test)
{
	...
	/* save header */
	ret = goodix_save_header(ts_test, GOODIX_RESULT_SAVE_PATH);

	/* save limits */
	ret = goodix_save_limits(ts_test, GOODIX_RESULT_SAVE_PATH);

	/* save data */
	ret = goodix_save_data(ts_test, GOODIX_RESULT_SAVE_PATH);

	/* save tail */
	ret = goodix_save_tail(ts_test, GOODIX_RESULT_SAVE_PATH);
}
#endif // SAVE_IN_CSV
