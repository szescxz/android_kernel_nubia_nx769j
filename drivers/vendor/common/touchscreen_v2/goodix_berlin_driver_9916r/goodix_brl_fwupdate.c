...
extern bool fw_update_low_flag;
extern bool fw_update_success_flag;
...
static int goodix_fw_version_compare(struct fw_update_ctrl *fwu_ctrl)
{
	...
	ts_info("active firmware version:%*ph", FW_VID_LEN, fw_version.patch_vid);
	ts_info("firmware file version: %*ph", FW_VID_LEN, fw_summary->fw_vid);
	if (ret < 0) {
		ts_info("active firmware version < firmware file version , update");
	...
	if (fwu_ctrl->ic_config && fwu_ctrl->ic_config->len > 0) {
		...
		ts_info("ic_cfg_id = 0x%x", ic_cfg_id);
		ts_info("file_cfg_id = 0x%x", file_cfg_id);
		if (ic_cfg_id != file_cfg_id) {
			ts_info("ic_cfg_id != file_cfg_id , update");
...
static int gtp_cfg_bin_upgrade(struct goodix_ts_core *core_data, const struct firmware *gtp_firmware)
{
...
static int gtp_fw_upgrade(struct ztp_device *cdev)
{
	struct fw_update_ctrl *fwu_ctrl = &goodix_fw_update_ctrl;
	int ret = 0;
	int mode = 0;

	fwu_ctrl->fw_data.firmware = cdev->tp_firmware;
...
int gtp_tp_fw_upgrade(struct ztp_device *cdev, char *fw_name, int fwname_len)
{
	...
	if (cdev->tp_firmware == NULL || cdev->tp_firmware->data == NULL) {
		ts_err("cdev->tp_firmware is NULL");
		return -EIO;;
	}
	if (cdev->tp_firmware->size > GOODIX_CFG_MAX_SIZE) {
		ts_info("gtp_fw_upgrade");
		ret = gtp_fw_upgrade(cdev);
		if (ret) {
			ts_err("gtp fw update failed");
		}
	} else {
		ts_info("gtp_cfg_bin_upgrade");
		ret = gtp_cfg_bin_upgrade(core_data, cdev->tp_firmware);
		if (ret) {
