...
bool fw_update_low_flag;
bool fw_update_success_flag;
...
static irqreturn_t goodix_ts_threadirq_func(int irq, void *data)
{
	...
	if (atomic_read(&core_data->suspended)) {
		ts_info("suspend in irq");
		while (count) {
			if (pm_resume_flag) {
				ts_info("pm resume , begin spi , time consuming is %d ms", (100 - count) * 5);
				break;
			}
...
#ifdef CONFIG_TOUCHSCREEN_IOVDD_GPIO_POWER
	if (core_data->board_data.iovdd_enable_gpio > 0) {
		ts_info("iovdd_enable gpio free");
		gpio_free(core_data->board_data.iovdd_enable_gpio);
	}
#else
...
static void goodix_ts_esd_work(struct work_struct *work)
{
	...
	if (ret) {
		...
		tpd_zlog_record_notify(TP_ESD_CHECK_WARN_NO);
#endif
		goodix_ts_power_off(cd);
		usleep_range(5000, 5100);
		goodix_ts_power_on(cd);
		r = cd->hw_ops->reset(cd, GOODIX_NORMAL_RESET_DELAY_MS);
		if (r) {
			ts_err("Failed to request reset gpio, r:%d", r);
		}
	}
...
out:
	...
	if (core_data->ztec.is_play_game) {
		...
		ret = hw_ops->set_sensibility(core_data, core_data->ztec.sensibility_level);
		ret = hw_ops->set_follow_hand_level(core_data, core_data->ztec.follow_hand_level);
		ret = hw_ops->set_display_rotation(core_data, core_data->ztec.display_rotation);
	}
	...
	if (fb_event && fb_event->data && core_data) {
		if (event == FB_EVENT_BLANK) {
			...
			if (*blank == FB_BLANK_UNBLANK)
				...
				ts_info("success in the goodix_ts_suspend one");
...
#if IS_ENABLED(CONFIG_PM)
	pm_resume_flag = 1;
	ts_info("%s: pm_resume_flag:%d", __func__, pm_resume_flag);
#endif
...
#if IS_ENABLED(CONFIG_PM)
/*process irq until master spi device resume*/
...
static SIMPLE_DEV_PM_OPS(dev_pm_ops, goodix_pm_suspend, goodix_pm_resume);
/*end*/
#endif/*CONFIG_PM*/
