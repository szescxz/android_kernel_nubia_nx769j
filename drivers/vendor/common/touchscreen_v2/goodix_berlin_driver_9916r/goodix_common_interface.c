...
static int tpd_init_tpinfo(struct ztp_device *cdev)
{
	...
	if (fw_update_low_flag && fw_update_success_flag) {
		snprintf(cdev->ic_tpinfo.fw_update_status, MAX_VENDOR_NAME_LEN, "low_update");
	}
...
void goodix_tpd_register_fw_class(struct goodix_ts_core *core_data)
{
	...
	zlog_tp_dev.device_name = "goodixvision";
