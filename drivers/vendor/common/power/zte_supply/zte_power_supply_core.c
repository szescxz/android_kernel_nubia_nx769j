...
EXPORT_SYMBOL(zte_power_supply_class);

ATOMIC_NOTIFIER_HEAD(zte_power_supply_notifier);
EXPORT_SYMBOL(zte_power_supply_notifier);
...
static bool __zte_power_supply_is_supplied_by(struct zte_power_supply *supplier,
...
{
	struct zte_power_supply *psy = data;
...
static void zte_power_supply_changed_work(struct work_struct *work)
{
	...
	if (likely(psy->changed)) {
		...
		class_for_each_device(zte_power_supply_class, NULL, psy,
				      __zte_power_supply_changed_work);
		atomic_notifier_call_chain(&zte_power_supply_notifier,
...
EXPORT_SYMBOL(zte_power_supply_changed);
...
static int zte_power_supply_populate_supplied_from(struct zte_power_supply *psy)
{
	...
	error = class_for_each_device(zte_power_supply_class, NULL, psy,
...
static int zte_power_supply_find_supply_from_node(struct device_node *supply_node)
{
	 ...
	 */
	error = class_for_each_device(zte_power_supply_class, NULL, supply_node,
...
EXPORT_SYMBOL(zte_power_supply_set_battery_charged);
...
EXPORT_SYMBOL(zte_power_supply_get_by_name);
...
EXPORT_SYMBOL(zte_power_supply_put);
...
EXPORT_SYMBOL(zte_power_supply_get_by_phandle);
						...
						void *data)
{
	...
	struct zte_power_supply **psy = param->psy;
 ...
 * @np: Pointer to device node holding phandle property.
 * @property: Name of property holding a power supply name.
 * @psy: Array of zte_power_supply pointers provided by the client which is
 * filled by power_supply_get_by_phandle_array.
 * @size: size of zte_power_supply pointer array.
 *
 * If power supply was found, it increases reference count for the
 * internal power supply's device. The user should power_supply_put()
 * after usage.
 ...
 */
int zte_power_supply_get_by_phandle_array(struct device_node *np,
				      const char *property,
				      struct zte_power_supply **psy,
				      ssize_t size)
{
	...
	param.psy_size = size;
	param.psy_count = 0;
	ret = class_for_each_device(zte_power_supply_class, NULL, &param,
...
EXPORT_SYMBOL(zte_power_supply_get_by_phandle_array);
...
EXPORT_SYMBOL(zte_devm_power_supply_get_by_phandle);
#endif /* CONFIG_OF */
...
EXPORT_SYMBOL(zte_power_supply_get_battery_info);
...
EXPORT_SYMBOL(zte_power_supply_put_battery_info);
...
EXPORT_SYMBOL(zte_power_supply_temp2resist_simple);
...
EXPORT_SYMBOL(zte_power_supply_ocv2cap_simple);
...
EXPORT_SYMBOL(zte_power_supply_find_ocv2cap_table);
...
EXPORT_SYMBOL(zte_power_supply_batinfo_ocv2cap);
...
EXPORT_SYMBOL(zte_power_supply_get_property);
...
EXPORT_SYMBOL(zte_power_supply_set_property);
...
EXPORT_SYMBOL(zte_power_supply_property_is_writeable);
...
EXPORT_SYMBOL(zte_power_supply_external_power_changed);

int zte_power_supply_powers(struct zte_power_supply *psy, struct device *dev)
{
	return sysfs_create_link(&psy->dev.kobj, &dev->kobj, "powers");
}
EXPORT_SYMBOL(zte_power_supply_powers);
...
EXPORT_SYMBOL(zte_power_supply_reg_notifier);
...
EXPORT_SYMBOL(zte_power_supply_unreg_notifier);
...
EXPORT_SYMBOL(zte_power_supply_register);
...
EXPORT_SYMBOL(zte_power_supply_register_no_ws);
...
EXPORT_SYMBOL(zte_devm_power_supply_register);
...
EXPORT_SYMBOL(zte_devm_power_supply_register_no_ws);
...
EXPORT_SYMBOL(zte_power_supply_unregister);

void *zte_power_supply_get_drvdata(struct zte_power_supply *psy)
{
	return psy->drv_data;
}
EXPORT_SYMBOL(zte_power_supply_get_drvdata);
