...
static int zte_power_supply_hwmon_in_to_property(u32 attr)
{
	switch (attr) {
...
static int zte_power_supply_hwmon_curr_to_property(u32 attr)
{
	switch (attr) {
...
static int zte_power_supply_hwmon_temp_to_property(u32 attr, int channel)
{
	if (channel) {
		switch (attr) {
...
static int
zte_power_supply_hwmon_to_property(enum hwmon_sensor_types type,
			       u32 attr, int channel)
{
	switch (type) {
	case hwmon_in:
		return zte_power_supply_hwmon_in_to_property(attr);
	case hwmon_curr:
		return zte_power_supply_hwmon_curr_to_property(attr);
	case hwmon_temp:
		return zte_power_supply_hwmon_temp_to_property(attr, channel);
	default:
		return -EINVAL;
	}
}

static bool zte_power_supply_hwmon_is_a_label(enum hwmon_sensor_types type,
					   u32 attr)
{
	return type == hwmon_temp && attr == hwmon_temp_label;
}

static bool zte_power_supply_hwmon_is_writable(enum hwmon_sensor_types type,
					   u32 attr)
{
	switch (type) {
	case hwmon_in:
...
static umode_t zte_power_supply_hwmon_is_visible(const void *data,
					     ...
					     u32 attr, int channel)
{
	...
	if (zte_power_supply_hwmon_is_a_label(type, attr))
		return 0444;

	prop = zte_power_supply_hwmon_to_property(type, attr, channel);
	if (prop < 0 || !test_bit(prop, psyhw->props))
		return 0;

	if (zte_power_supply_property_is_writeable(psyhw->psy, prop) > 0 &&
	    zte_power_supply_hwmon_is_writable(type, attr))
		return 0644;

	return 0444;
}

static int zte_power_supply_hwmon_read_string(struct device *dev,
					  enum hwmon_sensor_types type,
					  u32 attr, int channel,
					  const char **str)
{
...
static int
zte_power_supply_hwmon_read(struct device *dev, enum hwmon_sensor_types type,
			u32 attr, int channel, long *val)
{
	struct power_supply_hwmon *psyhw = dev_get_drvdata(dev);
	struct zte_power_supply *psy = psyhw->psy;
	...
	ret  = zte_power_supply_get_property(psy, prop, &pspval);
	if (ret)
		return ret;
...
static int
zte_power_supply_hwmon_write(struct device *dev, enum hwmon_sensor_types type,
			 u32 attr, int channel, long val)
{
	struct power_supply_hwmon *psyhw = dev_get_drvdata(dev);
	struct zte_power_supply *psy = psyhw->psy;
	...
	return zte_power_supply_set_property(psy, prop, &pspval);
}

static const struct hwmon_ops power_supply_hwmon_ops = {
	.is_visible	= zte_power_supply_hwmon_is_visible,
	.read		= zte_power_supply_hwmon_read,
	.write		= zte_power_supply_hwmon_write,
	.read_string	= zte_power_supply_hwmon_read_string,
};
...
static void zte_power_supply_hwmon_bitmap_free(void *data)
{
	bitmap_free(data);
}

int zte_power_supply_add_hwmon_sysfs(struct zte_power_supply *psy)
{
...
void zte_power_supply_remove_hwmon_sysfs(struct zte_power_supply *psy)
{
